...
module Minerva
  module Search
    class Sanitize
      ...
      def all_sql_fields
        ['resources.id, resources.learning_resource_type, resources.extensions, resources.version_scope'] + FieldMap.instance.field_map.values.map(&:select_sql).flatten.compact.uniq
      end
      ...
      def check_selection_field(attrs)
        ...
        if input_fields.all? { |k| valid_fields.keys.include?(k) }
          result[:fields] = (input_fields.map { |k| valid_fields[k]&.select_sql }.flatten.compact + ['resources.id', 'resources.learning_resource_type', 'resources.extensions', 'resources.version_scope']).uniq
