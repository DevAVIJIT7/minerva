      ...
      context 'with bad input valid params' do
        ...
        it 'throws error wrong sort' do
          expect(Search::Engine.new(sort: 'invalid_field', orderBy: 'asc').perform.warning)
            .to eq(:CodeMinor=>:invalid_sort_field, :Description=>"Use any of relevance, name, description, publisher, efficacy, learningResourceType, language, rating, publishDate, timeRequired, author for sorting parameter", :Severity=>:warning)
        end
