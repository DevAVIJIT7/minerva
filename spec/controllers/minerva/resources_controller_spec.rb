      ...
      context 'authorized user request' do
        describe 'filter' do
          ...
          describe 'searching efficacy' do
            before(:each) do
              ...
              stats = Alignments::ResourceStat.where(resource_id: resource.id)
                          .pluck(:taxonomy_ident, :effectiveness).inject({}) { |h, el| h.merge([el].to_h) }
              resource.update_columns(all_resource_stat_ids: Alignments::ResourceStat.where(resource_id: resource.id).pluck(:id),
                                      efficacy: stats)
            end
            ...
            context 'sorts by efficacy' do
              ...
              before do
                ...
                stats = Alignments::ResourceStat.where(resource_id: resource2.id)
                            .pluck(:taxonomy_ident, :effectiveness).inject({}) { |h, el| h.merge([el].to_h) }
                resource2.update_columns(all_resource_stat_ids: Alignments::ResourceStat.where(resource_id: resource2.id).pluck(:id),
                                         efficacy: stats)
              end
          ...
          context 'searches by learningObjectives' do
            ...
            context 'search by child ids' do
              it 'returns empty set' do
                root = FactoryBot.create(:taxonomy, identifier: 'CCSS.Math.Content.2.G')
                parent = FactoryBot.create(:taxonomy, identifier: 'CCSS.Math.Content.2.G.A', parent: root)
                s1.update(parent: parent)
                s2.update(parent: parent)
                params.merge!(limit: 1, filter: "learningObjectives.targetName='CCSS.Math.Content.2.G'")
                action.call
                expect(json_response['resources'].count).to eq(1)
                params.merge!(limit: 1, filter: "learningObjectives.targetName='CCSS.Math.Content.2.G.A'")
                action.call
                expect(json_response['resources'].count).to eq(1)
              end
            end
        ...
        context 'when searching for all fields' do
          ...
          it 'returns resources with all fields' do
            t = FactoryBot.create(:taxonomy, identifier: 'CCSS.Math.Content.2.G.A.1', min_age: 6, max_age: 7)
            FactoryBot.create(:alignment, resource: resource, taxonomy: t)
            resource.update_columns(min_age: 6, max_age: 7, all_resource_stat_ids: [resource_stat.id],
